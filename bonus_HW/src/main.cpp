...
int main(int argc, char** argv) {
  ...
  std::string input_path = "/vgg_data/cifar10/test_batch.bin";
  int data_offset = 0;
  int batch = 1;
  std::string parameter_path = "/vgg_data/vgg_weight/values_vgg.txt";
  // Read arguments
  if (argc == 1) {
    ...
    batch = atoi(argv[3]);
    parameter_path = argv[4];
  } else {
    std::cout << "[ERROR] Invalid arguments" << std::endl;
    std::cout
        << "Usage: ./predict INPUT_PATH DATA_OFFSET BATCH"
        << std::endl;
    std::cout << "    INPUT_PATH: path to input data, e.g. "
                 "/vgg_data/cifar10/test_batch.bin"
              << std::endl;
    std::cout << "    DATA_OFFSET: data_offset for input data, e.g. 0"
              << std::endl;
    std::cout << "    BATCH: batch size to inference, e.g. 1" << std::endl;
    std::cout << "    PARAMETER_PATH: path to parameter, e.g. /vgg_data/vgg_weight/values_vgg.txt"
              << std::endl;
    exit(-1);
  }
  // Show arguments
  ...
  int predict_cpu[batch] = {3, 9, 6, 0, 6, 6, 8, 7, 3, 8, 3, 9, 5, 7, 9, 5, 3, 3, 9, 6, 7, 0, 9, 9, 4, 5, 3, 0, 8, 6, 6, 5, 5, 3, 9, 9, 4, 8, 9, 5, 4, 6, 5, 6, 0, 9, 3, 9, 7, 6, 9, 8, 3, 3, 8, 7, 3, 3, 3, 3, 7, 3, 6, 3, 6, 5, 1, 5, 3, 7, 3, 3, 7, 9, 0, 5, 9, 3, 3, 9, 9, 1, 1, 5, 5, 5, 8, 3, 9, 9, 0, 6, 9, 6, 4, 6, 6, 0, 0, 7, 4, 5, 6, 3, 1, 1, 3, 6, 9, 7, 4, 0, 3, 5, 1, 3, 0, 4, 5, 5, 9, 3, 1, 5, 8, 9, 5, 3};
  ...
  //  std::cout << "[INFO] CPU  elapsed time is " << cudaElapsedTime << " msec" << std::endl;
  //}
  ...
  net_cuda->classify(predict_cuda, batch);  // softmax is left out from execution time
